...
/*Functions for scorecomplex
 * We only take C-alpha from matches.
 * Only Ms in Backtrace.
 * We need matchLen (the number of matches) to avoid overflow.
 * qStartPos and tStartPos are always 0.*/

void TMaligner::initQueryforComplexAln(float *x, float *y, float *z, unsigned int queryLen, unsigned int matchLen){
    memset(querySecStruc, 0, sizeof(char) * queryLen);
...
TMaligner::TMscoreResult TMaligner::computeComplexTMscore(float *x, float *y, float *z, unsigned int matchLen, const std::string &backtrace, unsigned int normalizationLen) {
    int qPos = 0;
    int tPos = 0;
    std::string cigarString = backtrace;
    std::fill(invmap, invmap+queryLen, -1);
    ...
    float Lnorm;         //normalization length
    float score_d8,d0,d0_search,dcu0;//for TMscore search
    parameter_set4search(normalizationLen, normalizationLen, D0_MIN, Lnorm, score_d8, d0, d0_search, dcu0);
    double prevD0_MIN = D0_MIN;// stored for later use
    int prevLnorm = Lnorm;
    double prevd0 = d0;
    double local_d0_search = d0_search;
    double TMalnScore = standard_TMscore(r1, r2, xtm, ytm, xt, targetCaCords, queryCaCords, queryLen, invmap,L_ali, rmsd0, D0_MIN, Lnorm, d0, score_d8, t, u,  mem);
    D0_MIN = prevD0_MIN;
    Lnorm = prevLnorm;
    d0 = prevd0;
    double TM = detailed_search_standard(r1, r2, xtm, ytm, xt, targetCaCords, queryCaCords, queryLen,invmap, t, u, 40, local_d0_search, true, Lnorm, score_d8, d0, mem);
    TM = std::max(TM, TMalnScore);
    return TMaligner::TMscoreResult(u, t, TM, rmsd0);
}
