...
void TMaligner::initQueryforComplexAln(float *x, float *y, float *z, char * querySeq, unsigned int queryLen, unsigned int matchLen){
    memset(querySecStruc, 0, sizeof(char) * queryLen);
...
TMaligner::TMscoreResult TMaligner::computeComplexTMscore(float *x, float *y, float *z, unsigned int targetLen, int qStartPos, int dbStartPos, const std::string &backtrace, unsigned int alnLen, unsigned int qLen) {
    int qPos = qStartPos;
    int tPos = dbStartPos;
    std::string cigarString = backtrace;
    std::fill(invmap, invmap+queryLen, -1);
...
//    memcpy(target_y, y, sizeof(float) * targetLen);
//    memcpy(target_z, z, sizeof(float) * targetLen);
    memcpy(target_x, x, sizeof(float) * qLen);
    memcpy(target_y, y, sizeof(float) * qLen);
    memcpy(target_z, z, sizeof(float) * qLen);
    Coordinates targetCaCords;
    targetCaCords.x = target_x;
    targetCaCords.y = target_y;
    targetCaCords.z = target_z;
